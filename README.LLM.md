...
│   ├── settings/
│   │   ├── settings.go          # Repo-local Fry settings (.fry-config/config.json): Load, Save, GetEngine, SetEngine
...
│   │   ├── semantic.go          # RunSemanticScan: LLM-powered codebase analysis → .fry-config/codebase.md
...
type Epic struct {
    ...
    MCPConfig                string         // path to MCP server config (Claude only)
...
### Prompt Layering (assembled in `sprint/prompt.go`)

| Layer | Content | Notes |
|-------|---------|-------|
| 0.5 | Codebase context | `.fry-config/codebase.md` (optional, generated by `fry init`) |
| 0.75 | Codebase memories | `.fry-config/codebase-memories/*.md` (optional, 10KB cap) |
...
| 1.7 | Mid-build steering | user directive injected mid-build via `.fry/agent-directive.md` (optional) |
| — | Active divergences | intentional divergences from `.fry/deviation-log.md` (optional, sprint-filtered) |
...
| `AuditSessionMaxCalls` | `3` | Max same-role audit session calls before refresh |
| `FixSessionMaxCalls` | `4` | Max same-role fix session calls before refresh |
| `MaxFixFileInlineBytes` | `8000` | Max bytes of target file content inlined per file in fix prompt |
...
| `ProjectConfigFile` | `.fry-config/config.json` | Repo-local Fry settings (currently self-improve engine) |
